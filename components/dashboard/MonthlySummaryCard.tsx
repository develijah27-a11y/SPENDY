...
export function MonthlySummaryCard() {
  ...
  return (
    <section aria-label="Monthly Financial Summary" className="grid grid-cols-1 sm:grid-cols-3 gap-3">
      {/* Income */}
      <article
        aria-label={`${currentMonthName} Income: ${formatUGX(monthlyIncome)}`}
        className="rounded-3xl glass-panel p-4 sm:p-5 border border-black/15 dark:border-white/20 relative overflow-hidden group hover:border-emerald-500/40 transition-colors shadow-md"
      >
      ...
      {/* Expenses */}
      <article
        aria-label={`${currentMonthName} Expenses: ${formatUGX(monthlyExpenses)}`}
        className="rounded-3xl glass-panel p-4 sm:p-5 border border-black/15 dark:border-white/20 relative overflow-hidden group hover:border-red-500/40 transition-colors shadow-md"
      >
        <div className="flex items-center justify-between">
      ...
      {/* Net Saved */}
      <article
        aria-label={`Money Saved: ${formatUGX(netSavings)}, ${savingsRate}% net savings rate`}
        className="rounded-3xl glass-panel p-4 sm:p-5 border border-black/15 dark:border-white/20 relative overflow-hidden group hover:border-blue-500/40 transition-colors shadow-md"
      >
        <div className="flex items-center justify-between">
          ...
          <div className="w-8 h-8 rounded-xl bg-blue-500/20 text-blue-600 dark:text-blue-400 flex items-center justify-center shadow-sm">
            <PiggyBank className="w-4 h-4 font-black" aria-hidden="true" />
          </div>
        </div>
        <p className="text-xl sm:text-2xl font-black text-gray-950 dark:text-white mt-2 tracking-tight">
