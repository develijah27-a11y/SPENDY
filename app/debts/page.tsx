  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {/* Money I Owe */}
        <div className="rounded-2xl p-5 sm:p-6 border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0F172A]">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-rose-500">Money I Owe (To Repay)</span>
            <div className="w-8 h-8 rounded-xl bg-rose-500/10 text-rose-500 flex items-center justify-center font-bold">
              <ArrowDownLeft className="w-4 h-4" />
            </div>
          </div>
          <h2 className="text-3xl sm:text-4xl font-black text-slate-950 dark:text-white mt-2 font-mono tabular-nums">{formatUGX(totalIOweRemaining)}</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Across {debtsIOwe.length} active commitments</p>
        </div>

        {/* Money Owed To Me */}
        <div className="rounded-2xl p-5 sm:p-6 border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0F172A]">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400">Money Owed To Me (To Collect)</span>
            <div className="w-8 h-8 rounded-xl bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 flex items-center justify-center font-bold">
              <ArrowUpRight className="w-4 h-4" />
            </div>
          </div>
          <h2 className="text-3xl sm:text-4xl font-black text-slate-950 dark:text-white mt-2 font-mono tabular-nums">{formatUGX(totalOwedToMeRemaining)}</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Across {debtsOwedToMe.length} active loans given</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 text-xs">
        <button
          ...
          className={`px-3.5 py-2 rounded-xl font-bold transition-all cursor-pointer ${
            activeTab === 'all' ? 'bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 border border-emerald-500/30' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700'
          }`}
        >
